...
```{r echo=T}
View(df_clean %>% filter(shot_dist < closest_defender_dist, shot_dist > 0) %>% select(shot_result, shot_dist, closest_defender_dist, everything()))

# free to basket
df_clean %>% filter(shot_dist < closest_defender_dist, shot_dist > 0) %>% summarise(mean(shot_result == "made"))

# unguarded
df_clean %>% filter(closest_defender_dist >= 7, shot_dist <= 7) %>% summarise(mean(shot_result == "made"))
df_clean %>% filter(closest_defender_dist >= 7, shot_dist %in% c(7, 14)) %>% summarise(mean(shot_result == "made"))
df_clean %>% filter(closest_defender_dist >= 7, shot_dist %in% c(14, 23.75)) %>% summarise(mean(shot_result == "made"))
df_clean %>% filter(closest_defender_dist >= 7, pts_type == 3) %>% summarise(mean(shot_result == "made"))
...
# tightly guarded
df_clean %>% filter(closest_defender_dist < 3, shot_dist <= 7) %>% summarise(mean(shot_result == "made"))
df_clean %>% filter(closest_defender_dist < 3, shot_dist %in% c(7, 14)) %>% summarise(mean(shot_result == "made"))
df_clean %>% filter(closest_defender_dist < 3, shot_dist %in% c(14, 23.75)) %>% summarise(mean(shot_result == "made"))
df_clean %>% filter(closest_defender_dist < 3, pts_type == 3) %>% summarise(mean(shot_result == "made"))


shot_pocket <- function(df, vec_def_dist, vec_shot_dist) {
  # def_dist and shot_dist need to be vectors describing an interval
  stopifnot(lengths(vec_def_dist == 2) & lengths(vec_shot_dist == 2))
    df %>% 
    subset(closest_defender_dist >= vec_def_dist[1]) %>%
    subset(closest_defender_dist < vec_def_dist[2]) %>%
    subset(shot_dist >= vec_shot_dist[1]) %>%
    subset(shot_dist < vec_shot_dist[2]) %>%
    summarise(mean(shot_result == "made")
              )
}


add_dummy <- function(df, name_dummy, 
                      name_var1, range_var1, 
                      name_var2, range_var2) {
  stopifnot(lengths(range_var1 == 2) & lengths(range_var2 == 2))
  stopifnot(c(name_var1, name_var2) %in% names(df))
  df[name_dummy] <- 
    ifelse(
      df[name_var1] >= min(range_var1) & df[name_var1] <= max(range_var1) &
      df[name_var2] >= min(range_var2) & df[name_var2] <= max(range_var2),
      1,0
      )
  df[[name_dummy]]
}
...
def_dist_list <-  
  list(
    free = c(7, 99),
    contested = c(3, 7),
    closely_contested = c(0, 3)
)

df_dummy_names <- 
  expand.grid(shot_dist=names(shot_dist_list), 
              def_dist= names(def_dist_list), stringsAsFactors = T) %>% 
  transmute(name = paste(shot_dist, def_dist, sep = "_"))

distances <- expand.grid(shot_dist=shot_dist_list, def_dist=def_dist_list, stringsAsFactors = F)

dummies <-  mapply(add_dummy, 
                   name_dummy = df_dummy_names$name, 
                   range_var1 = distances$shot_dist, 
                   range_var2 = distances$def_dist, 
                   MoreArgs = list(df = df_clean, 
                                   name_var1 = "shot_dist", 
                                   name_var2 = "closest_defender_dist")
                   ) %>% 
  as.data.frame()

df_combined <- cbind(df_clean, dummies)



shot_pocket(df_clean, c(0,10), c(4, 10))



mapply(shot_pocket, vec_def_dist = distances$def_dist, vec_shot_dist = distances$shot_dist, MoreArgs = list(df = df_clean))
...
model_specs$regression <- 
  ...
  dribbles +
  home_game +
  high_pct_scorer + high_pct_scorer:shot_dist +
  low_pct_scorer + low_pct_scorer:shot_dist +
  high_pct_defender + high_pct_defender:closest_defender_dist +
  low_pct_defender + low_pct_defender:closest_defender_dist


tic()
grids$lasso <- expand.grid(alpha = 1,lambda = 10 ^ seq(-5, -2, length = 4))
set.seed(111)
