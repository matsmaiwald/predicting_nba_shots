...
grids$ridge <- expand.grid(alpha = 0,lambda = seq(0, 1, length = 10))
...
```{r}
confusionMatrix(model_predictions$ridge,df_test$fgm, positive = "1")
...
```{r}
confusionMatrix(model_predictions$lasso,df_test$fgm, positive = "1")
...
```{r}
confusionMatrix(model_predictions[["knn"]],df_test$fgm, positive = "1")
