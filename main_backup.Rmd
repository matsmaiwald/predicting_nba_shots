...
```{r echo=T}
rpart.plot(fitted_models$cart$finalModel, box.palette="RdBu", shadow.col="gray", nn=TRUE)
...
```{r echo=T}
model_specs$bagged <- shot_result ~ shot_dist + pts_type + 
                                closest_defender_dist + 
                                shot_clock +
                                final_margin +
                                touch_time +
...
rf_control <- trainControl(classProbs = TRUE,
                            summaryFunction = twoClassSummary) 
...
model_predictions[["bagged"]] <-  predict(fitted_models[["bagged"]], df_test)
toc()
```

```{r echo=T}
confusionMatrix(model_predictions[["bagged"]],df_test$shot_result, positive = "made")
```
